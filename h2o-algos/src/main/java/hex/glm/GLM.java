...
public class GLM extends ModelBuilder<GLMModel,GLMParameters,GLMOutput> {
  ...
  private double [] _xval_deviances_generate_SH;// store cv average deviance for generate_scoring_history
  private double [] _xval_sd_generate_SH; // store the standard deviation of cv for generate_scoring_historty
  private boolean _insideCVCheck = false; // detect if we are running inside CV model summary
  /**
  ...
  private void generateCVScoringHistory(ModelBuilder[] cvModelBuilders) {
    ...
    for (int index = 0; index < devianceTestLength; index++) {
      ...
      for (int modelIndex = 0; modelIndex < cvModelBuilders.length; modelIndex++) {
        ...
        if (_parms._lambda_search) {
          testDev += g._lambdaSearchScoringHistory._lambdaDevTest.get(index);
          testDevSq += g._lambdaSearchScoringHistory._lambdaDevTest.get(index) * 
                  g._lambdaSearchScoringHistory._lambdaDevTest.get(index);;
        } else {
          testDev += g._scoringHistory._lambdaDevTest.get(index);
          testDevSq += g._scoringHistory._lambdaDevTest.get(index) * g._scoringHistory._lambdaDevTest.get(index);
        }
      }
      double testDevAvg = testDev / cvModelBuilders.length;
      double testDevSE = testDevSq - testDevAvg * testDevAvg;
      _xval_sd_generate_SH[index] = Math.sqrt(testDevSE/((cvModelBuilders.length - 1) * cvModelBuilders.length));
      _xval_deviances_generate_SH[index] = testDevAvg;
    }
  }
  ...
  static class LambdaSearchScoringHistory {
    ...
    public synchronized void addLambdaScore(int iter, int predictors, double lambda, double devRatioTrain, 
                                            ...
                                            double alpha) {
      if (_lambdaIters.size() > 0 && (iter <= _lambdaIters.get(_lambdaIters.size()-1)))  {  // no duplicated records
          return;
      }
      _scoringTimes.add(System.currentTimeMillis());
...
//      _scoreIterationList.add(_state._iter);
      ...
      if (_parms._lambda_search)
        ...
        _model._output._scoring_history = _scoringHistory.to2dTable(_parms, _xval_deviances_generate_SH, _xval_sd_generate_SH);
      if (!(mtrain == null)) {
      ...
      if (_parms._generate_scoring_history) { // update scoring history with deviance train and valid if available
        double xval_deviance = -1;
        double xval_se = -1;
        if (_xval_deviances_generate_SH != null && _lambdaSearchScoringHistory._alphas.size() <
                _xval_deviances_generate_SH.length) {
          xval_deviance = _xval_deviances_generate_SH[_lambdaSearchScoringHistory._alphas.size()];
          xval_se = _xval_sd_generate_SH[_lambdaSearchScoringHistory._alphas.size()];
        }
        if (!(mtrain == null) && !(_valid == null)) {
          ...
          if (_parms._lambda_search) {
            _lambdaSearchScoringHistory.addLambdaScore(_state._iter, ArrayUtils.countNonzeros(_state.beta()),
                    _state.lambda(), _state.deviance() / mtrain._nobs, -1, xval_deviance,
                    xval_se, _state.alpha());
          } else {
            _scoringHistory.addIterationScore(!(mtrain == null), !(_valid == null), _state._iter, _state.likelihood(),
                    _state.objective(), _state.deviance(), -1, mtrain._nobs, 1, _state.lambda(),
                    _state.alpha());
          }
        }
...
/*      if (_vcov != null) {
    ...
    public boolean progress(double[] beta, double likelihood) {
      ...
      if ((!_parms._lambda_search || (_parms._generate_scoring_history && !_insideCVCheck)) && (_parms._score_each_iteration ||
              timeSinceLastScoring() > _scoringInterval || ((_parms._score_iteration_interval > 0) &&
              ((_state._iter % _parms._score_iteration_interval) == 0))))
        updateProgress(true);
    ...
    protected void updateProgress(boolean canScore) {
      ...
      if (!_parms._generate_scoring_history && !_parms._lambda_search) { // update as before
